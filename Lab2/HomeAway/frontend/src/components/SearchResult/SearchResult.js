...
render(){
    ...
    let details=currentProps.map((kkdata,i) =>{
        
        // console.log(kkdata.imagename);
        //         axios.post('http://localhost:3001/download/'+kkdata.imagename)
        //         .then(response =>
        //         {
        ...
        return (
            ...
            <Link to={{pathname:"/displayProperty",state:{data:this.state.data,value:i,guests:myguests,arrive_date:myarrive_date,checkout_date:mycheckout_date,curr_page:mycurr_page,propPerPage:mypropPerPage}, target:"_blank"}} style={{ textDecoration: 'none' }}><div className="col-md-2point5" style={{'margin':'0px'}}>
                            <div >
    ...
    const pageNumbers=[];
    for(let i=1;i<=Math.ceil(data.length/propPerPage);i++){
        pageNumbers.push(i)
    }
